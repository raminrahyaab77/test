...
export default function CertificateCard({ certificate, onDownload }) {
  const earnedDate = certificate.earnedDate ? new Date(certificate.earnedDate) : null;
  const earnedDateText = earnedDate && !isNaN(earnedDate) ? earnedDate.toLocaleDateString() : 'N/A';
      ...
      <div style={{ marginBottom: '16px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
          ...
          <span style={{ fontSize: '14px', fontWeight: '600', color: '#15803D' }}>
            {certificate.score !== null && certificate.score !== undefined ? `${certificate.score}%` : 'N/A'}
          </span>
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
