...
import { generateCertificatePDF } from '../../utils/certificates/pdf';
...
export default function Certificates() {
  ...
  const normalizeCertificateData = (certificate) => {
    const normalizedScore = normalizePercentage({
    ...
    return {
      ...
      score: normalizedScore, // integer in 0-100 or null
      earnedDate: certificate?.earnedDate || certificate?.completedAt || certificate?.createdAt || null,
  ...
  // Debug logs
  // Ensure we see final normalized values
  console.log('=== CERTIFICATE PERCENTAGE SCORES (normalized to 0-100) ===');
  certificates.forEach((cert) => {
    console.log(`Certificate: ${cert.examTitle} - Score: ${cert.score !== null && cert.score !== undefined ? cert.score : 'N/A'}%`);
  });

  console.log('=== PASSED ATTEMPTS PERCENTAGE SCORES (normalized to 0-100) ===');
  passedAttempts.forEach((attempt) => {
    console.log(`Attempt: ${attempt.examTitle} - Score: ${attempt.score !== null && attempt.score !== undefined ? attempt.score : 'N/A'}%`);
  });
  ...
  const handleViewCertificate = (cert) => {
    ...
    try {
      generateCertificatePDF({ certificate, userFullName: user?.fullName || 'User' });
        ...
        {isLoading ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px' }}>
            {certificates.map((certificate) => (
              <CertificateCard key={certificate.id} certificate={certificate} onDownload={handleViewCertificate} />
            ))}
          </div>
        )}
      </div>
